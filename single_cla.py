...
with open("data_1_1.csv", encoding='utf-8') as f:
...
y_train, y_test = scores[idxes[:300]], scores[idxes[300:]]
...
class Bert_Model(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        #output[0]: torch.Size([batch_size, 300, 768])最后の層　outputs[1]はels:torch.Size([batch_size, 768])
...
if os.path.exists("./model1_1.pkl"):
   model = torch.load("./model1_1.pkl")
...
def train(epoch):
    for i in range(epoch):
        ...
        for idx, (ids, att, y) in enumerate(train_loader):
            ...
            if (idx + 1) % 1 == 0:  # 设定每个batch中的多少个step打印一次结果。
                print("Epoch {:04d} | Step {:04d}/{:04d} | Loss {:.8f}".format(
                    i + 1, idx + 1, len(train_loader), train_loss_sum / (idx + 1)))
        acc = evaluate(model)
        print(acc)
    torch.save(model, "./model1_1.pkl")  #保存整个模型
